...
class ReplicaMap {
  ...
  private final ConcurrentHashMap<String, ConcurrentHashMap<Block, ReplicaInfo>> map_nonlock =
    new ConcurrentHashMap<String, ConcurrentHashMap<Block, ReplicaInfo>>();
  ...
  String[] getBlockPoolList() {
    try (AutoCloseableLock l = lock.acquire()) {
      return map_nonlock.keySet().toArray(new String[map_nonlock.keySet().size()]);   
    }
  }
  ...
  ReplicaInfo get(String bpid, long blockId) {
    checkBlockPool(bpid);
    try (AutoCloseableLock l = lock.acquire()) {
      ConcurrentHashMap<Block, ReplicaInfo> m = map_nonlock.get(bpid);
  ...
  ReplicaInfo get_nolock(String bpid, Block block) {
	    checkBlockPool(bpid);
	    checkBlock(block);
	    ReplicaInfo replicaInfo = get_nolock(bpid, block.getBlockId());
	    if (replicaInfo != null && 
	        block.getGenerationStamp() == replicaInfo.getGenerationStamp()) {
	      return replicaInfo;
	    }
	   ...
	   * @return the replica's meta information
	   */
	  ReplicaInfo get_nolock(String bpid, long blockId) {
	    checkBlockPool(bpid);
  ...
  ReplicaInfo add(String bpid, ReplicaInfo replicaInfo) {
    ...
    try (AutoCloseableLock l = lock.acquire()) {
      ...
      if (m == null) {
        ...
        m = new ConcurrentHashMap<Block, ReplicaInfo>();
        map_nonlock.put(bpid, m);
      }
      return  m.put(new Block(replicaInfo.getBlockId()),replicaInfo);
    }
  }
  ...
  void addAll(ReplicaMap other) {
	  map_nonlock.putAll(other.map_nonlock);
  }
  ...
  int size(String bpid) {
    ConcurrentHashMap<Block, ReplicaInfo> m = null;
    try (AutoCloseableLock l = lock.acquire()) {
      m = map_nonlock.get(bpid);
