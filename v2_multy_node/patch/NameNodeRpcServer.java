...
class NameNodeRpcServer implements NamenodeProtocols {
  ...
  private final BlockReportProcessingThread processiong;
	private final LinkedBlockingQueue<Runnable> processiong_queue;

  public NameNodeRpcServer(Configuration conf, NameNode nn)
      throws IOException {
    this.nn = nn;
    this.namesystem = nn.getNamesystem();
    this.processiong_queue=new LinkedBlockingQueue<>(1024);
    this.processiong=new BlockReportProcessingThread(this.namesystem, this.processiong_queue);
    try {
    	this.processiong.start();
    }catch(Throwable e)
    {
    	LOG.error("processiong",e);
    }
  ...
  @Override // DatanodeProtocol
  public void blockReceivedAndDeleted(final DatanodeRegistration nodeReg, String poolId,
			StorageReceivedDeletedBlocks[] receivedAndDeletedBlocks) throws IOException {
		verifyRequest(nodeReg);
		metrics.incrBlockReceivedAndDeletedOps();
		if (blockStateChangeLog.isDebugEnabled()) {
			blockStateChangeLog.debug("*BLOCK* NameNode.blockReceivedAndDeleted: " + "from " + nodeReg + " "
		...
		for (StorageReceivedDeletedBlocks r : receivedAndDeletedBlocks) {
			final StorageReceivedDeletedBlocks rr = r;
			this.enqueueBlockOp(new Runnable() {
				@Override
				public void run() {
					try {
						bm.processIncrementalBlockReport(nodeReg, rr);
					} catch (Exception ex) {
						blockStateChangeLog.error("*BLOCK* NameNode.blockReceivedAndDeleted: " + "failed from "
								+ nodeReg + ": " + ex.getMessage());
					}
				}
			});
